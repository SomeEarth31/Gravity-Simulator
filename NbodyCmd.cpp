...
To run the program go to the folder with the .cpp file and open terminal there. There type the following commands:
> make NbodyCmd.cpp
#For gravity sim
> ./NbodyCmd -g <CSV_filename> <time_measure> <time_step>
#For electrostatic sim
> ./NbodyCmd -g <CSV_filename> <time_measure>
...
*/
int start(std::string sim_type, std::string file_name, std::string time_measure, std::string time__step)
{
    int N = 0;
...
//  std::cout << N << std::endl;
    ...
    if (sim_type == "-g")
    {
        ...
        int T = stoi(time_measure);
        double time_step = 1.0 * stoi(time__step);
   ...
   // std::cout << Par[1].P() << " | " << Par[1].V() << std::endl;
    ...
    // getchar();
        ...
        std::cout << "Improper Argument use the folowing format: \n" << std::endl;
        std::cout << "For gravity sim: -g <CSV_input_filename> <time_to_measure(in days)";
        std::cout << " <time_step> \n" << std::endl;
        std::cout << "For electrostatic sim: -e <CSV_input_filename> <time_to_measure(in 10^-20s)>"
                  << std::endl;
        return 0;
    }
...
int main(int argc, char* argv[]){
    if(argc>1){
        ...
        if((sim_type == "-g")|| (sim_type == "-e"))
        {
            //std::string sim_type(argv[1]);
            std::string file_name(argv[2]);
            std::string time_measure(argv[3]);
            std::string time_step(argv[4]);
            Exec_Time(&start, sim_type, file_name, time_measure, time_step);
        }
        else if(sim_type == "-h")
        {
            std::cout << "This is an N-Body Simulator\n \n";
            std::cout << "Use the folowing format: \n"
                      << std::endl;
            std::cout << "For gravity sim: -g <CSV_input_filename> <time_to_measure(in days)";
            std::cout << " <time_step> \n"
                      << std::endl;
